...
<head>
	...
	<title>Tamu PTSP | Pengambilan AC</title>
	      ...
	      <div class="row mb-2">
	        ...
	        <div class="col-sm-6">
	          <ol class="breadcrumb float-sm-right">
	            ...
	            <li class="breadcrumb-item active">Pengambilan Akta Cerai</li>
	          </ol>
	        </div>
	      </div>
	      <div class="row mb-2">
	        <div class="col-sm-2">
	          <a href="ac_tambah" class="btn btn-block bg-gradient-primary">
	      ...
	      <div class="row">
	        <div class="col-md-12">
	          <!-- <div class="card card-primary">
	            ...
	            <div class="card-body"> -->
	            <div class="card card-primary">
	              <table id="dt_ac" class="table table-bordered table-hover">
	                <thead>
	                  <tr>
	                    <th></th>
	                    <th>NO</th>
...
<script type="text/javascript">
	...
	function hapusData(id)
	  {
	    $.ajax({
	      url: "<?php echo base_url('produk/ac_hapus/') ?>"+id,
	      dataType: "text",
	...
	$(document).ready(function(){
		...
		dt_ac = $("#dt_ac").DataTable({
			...
			"order" : [[1, "asc"]],
			"ajax" : {
				"url" : "<?php echo base_url(); ?>produk/data_ac",
				"dataSrc" : "",
			},
			"columns" : [
			{"data" : "id"},
			...
			// }},
			{"data" : null, "sortable" : false, render: function(data,type,row,meta){
				return "<a href='<?php echo base_url('upload/ac/')?>"+row['foto']+"' data-lightbox="+row['foto']+" data-title='"+row['nama']+"'><img src='<?php echo base_url('upload/ac/')?>"+row['foto']+"' width='64' loading='lazy' /></a>";
			}},
			{"data" : null, "sortable" : false, render: function(data,type,row,meta){
				return "<a href='<?php echo base_url('produk/ac_ubah/') ?>"+row['id']+"' class='btn btn-warning'><i class='fas fa-edit'></i>Ubah</a>";
			}},
			{"data" : null, "sortable" : false, render: function(data,type,row,meta){
		...
		$("#dt_ac tbody").on('click', 'tr .deleteButton', function(e){
		  e.preventDefault();
		  // var currentRow = $(this).closest("tr");
		  var currentRow = $(this).closest('li').length ? $(this).closest('li') : $(this).closest('tr');
		  var data = $("#dt_ac").DataTable().row(currentRow).data();
